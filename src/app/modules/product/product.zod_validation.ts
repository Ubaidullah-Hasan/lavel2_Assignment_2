...
const productSchemaValidationZod = z.object({
    ...
    price: z.number({required_error: "Price must be required!"}).min(0, "Price must be a positive number"),
    category: z.string().trim().nonempty("Category is required"),
    tags: z.array(z.string().trim().nonempty("Tag must be a non-empty string")).nonempty("Tags are required"),
    ...
    inventory: z.object({
        ...
        inStock: z.boolean(),
    }).refine(data => data.quantity !== undefined && data.inStock !== undefined, {
